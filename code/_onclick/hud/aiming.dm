...
/datum/component/aiming/proc/show_ui(mob/user, mob/target, stage)
	...
	var/list/possible_actions = list(SHOOT, RAISE_HANDS, DROP_TO_FLOOR, DROP_WEAPON)
...
/datum/component/aiming/proc/act(choice)
	...
	if(!choice)
		stop_aiming()
		return
	if(choice != SHOOT) // Handling voiceline cooldowns and mimes
...
/// Shows a big flashy exclamation mark above the target
